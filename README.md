...
| **FI / FI-AA** | 总账科目、FB50、财务凭证、收付款、清账、冲销、余额、科目确定、OB52 过账期间控制；固定资产主数据、购置资本化、资产浏览器、月度折旧 |
...
- **OB52：** 公司代码/年度一旦配置期间范围，FI 凭证仅可在启用的月份过账；未配置年度保持开放，兼容初始化和跨年运行。
...
unzip release/sap-1.0.0.zip
cd sap-1.0.0
./start.sh
```

浏览器访问 `http://127.0.0.1:8085`。默认账号（如启用登录）`admin / admin123`。
