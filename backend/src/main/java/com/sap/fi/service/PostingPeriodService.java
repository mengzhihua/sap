...
public class PostingPeriodService {
    ...
    public List<PostingPeriod> list() {
        return periods.selectList(new LambdaQueryWrapper<PostingPeriod>()
                .orderByDesc(PostingPeriod::getFiscalYear)
                .orderByAsc(PostingPeriod::getBukrs)
                .orderByAsc(PostingPeriod::getFromPeriod));
    }
    ...
    public PostingPeriod save(PostingPeriod p) {
        ...
        if (companyCodes.selectById(p.getBukrs()) == null) {
            throw new BizException("公司代码不存在: " + p.getBukrs());
        }
        ...
        if (duplicate != null) {
            throw new BizException("相同公司、年度和期间范围已存在");
        }
        if (p.getId() == null) {
            periods.insert(p);
    ...
    public void delete(long id) {
        if (periods.deleteById(id) == 0) {
            throw new BizException("过账期间配置不存在: " + id);
        }
    }

    public void assertOpen(String bukrs, LocalDate date) {
        if (date == null) throw new BizException("过账日期不能为空");
        List<PostingPeriod> configured = periods.selectList(new LambdaQueryWrapper<PostingPeriod>()
                .eq(PostingPeriod::getBukrs, bukrs)
                .eq(PostingPeriod::getFiscalYear, date.getYear()));
        if (configured.isEmpty()) {
            return;
        }
        int month = date.getMonthValue();
        if (configured.stream().noneMatch(p -> Boolean.TRUE.equals(p.getOpen())
                && month >= p.getFromPeriod() && month <= p.getToPeriod())) {
    ...
    private void validate(PostingPeriod p) {
        if (p == null) {
            throw new BizException("过账期间配置不能为空");
        }
        if (p.getBukrs() == null || p.getBukrs().trim().isEmpty()) {
            throw new BizException("公司代码不能为空");
        }
        if (p.getFiscalYear() == null || p.getFiscalYear() < 2000 || p.getFiscalYear() > 9999) {
            throw new BizException("会计年度不正确");
        }
        if (p.getFromPeriod() == null || p.getToPeriod() == null
                || p.getFromPeriod() < 1 || p.getToPeriod() > 12
                || p.getFromPeriod() > p.getToPeriod()) {
            throw new BizException("期间范围必须为 1 至 12，且起始期间不能晚于结束期间");
        }
        if (p.getOpen() == null) {
            p.setOpen(true);
        }
