...
public class FiTest extends TestSupport {
    ...
    void ob52RejectsPostingOutsideOpenRange() {
        ...
        period.setBukrs("1000"); period.setFiscalYear(2025);
        period.setFromPeriod(1); period.setToPeriod(3); period.setOpen(true);
        postingPeriods.insert(period);
        FiDocumentRequest request = new FiDocumentRequest();
        request.setBukrs("1000"); request.setBudat(java.time.LocalDate.of(2025, 4, 1));
        FiDocumentRequest.Item debit = new FiDocumentRequest.Item();
        debit.setSaknr("6401"); debit.setShkzg("S"); debit.setAmount(new java.math.BigDecimal("10"));
        FiDocumentRequest.Item credit = new FiDocumentRequest.Item();
        credit.setSaknr("1002"); credit.setShkzg("H"); credit.setAmount(new java.math.BigDecimal("10"));
    ...
    @Test
    void fb50UsesRequestedHeaderAndOpenPostingDate() {
        FiDocumentRequest request = new FiDocumentRequest();
        request.setBukrs("1000"); request.setWaers("USD");
        request.setBudat(java.time.LocalDate.of(2025, 2, 15));
        request.setBldat(java.time.LocalDate.of(2025, 2, 10));
        request.setHeaderText("Month-end adjustment"); request.setRefNo("REF-2025-02");
        FiDocumentRequest.Item debit = new FiDocumentRequest.Item();
        debit.setSaknr("6401"); debit.setShkzg("S"); debit.setAmount(new java.math.BigDecimal("10"));
        FiDocumentRequest.Item credit = new FiDocumentRequest.Item();
        credit.setSaknr("1002"); credit.setShkzg("H"); credit.setAmount(new java.math.BigDecimal("10"));
        request.setItems(Arrays.asList(debit, credit));
        com.sap.fi.entity.AccountingDocument saved = service.post(request);
        assertEquals("2025", saved.getGjahr());
        assertEquals("USD", saved.getWaers());
        assertEquals("Month-end adjustment", saved.getHeaderText());
        assertEquals("REF-2025-02", saved.getRefNo());
    }
}
