  ...
  <el-container class="shell">
    <el-header class="topbar">
      ...
      <el-dropdown class="user" @command="onUserCommand">
        <span class="user-link">{{ auth.user?.realName || auth.user?.username }} <el-icon><ArrowDown /></el-icon></span>
...
function suggest(query, cb) {
  basisApi.tcodes(query).then((rows) => cb((rows || []).map((x) => ({ value: `${x.tcode} – ${x.name}`, item: x })))).catch(() => cb([]))
}
