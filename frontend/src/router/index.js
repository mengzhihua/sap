...
export const groups = [
  ...
  { name: 'Basis 基础管理', children: [
    ...
    ['SE16', '事务代码目录', '/basis/tcodes', () => import('../views/basis/TcodeList.vue')],
...
const routeRecords = groups.flatMap((group) => group.children.map(([tcode, title, path, component]) => ({
  path: path.substring(1), component, meta: { tcode, title }
})))
const routes = [
  { path: '/login', component: Login },
  { path: '/', component: Layout, redirect: '/launchpad', children: [
    ...
    { path: 'mm/po/create', component: () => import('../views/mm/PurchaseOrderCreate.vue'), meta: { tcode: 'ME21N', title: '创建采购订单' } },
    ...routeRecords
  ]}
]
